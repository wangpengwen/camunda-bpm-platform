...
public class SetExternalTasksRetriesTest extends AbstractAsyncOperationsTest {
  ...
  public void shouldUpdateRetriesByExternalTaskIdArray() {
    ...
    // when
    Batch batch = externalTaskService.updateRetries().externalTaskIds(externalTaskIds.toArray(new String[externalTaskIds.size()])).setAsync(RETRIES);
    executeSeedAndBatchJobs(batch);
  ...
  public void shouldUpdateRetriesByProcessInstanceIdArray() {
    ...
    // when
    Batch batch = externalTaskService.updateRetries().processInstanceIds(processInstanceIds.toArray(new String[processInstanceIds.size()])).setAsync(RETRIES);
    executeSeedAndBatchJobs(batch);
